#!/bin/bash
passwordx=$(cat ~/hungry-recon/tools/.creds | grep password | awk {'print $3'})
dns_server=$(cat ~/hungry-recon/tools/.creds | grep 'dns_server' | awk {'print $3'})
xss_hunter=$(cat ~/hungry-recon/tools/.creds | grep 'xss_hunter' | awk {'print $3'})

big_ports=4443,2075,2076,6443,3868,3366,8443,8080,9443,9091,3000,8000,5900,8081,6000,10000,8181,3306,5000,4000,8888,5432,15672,9999,161,4044,7077,4040,9000,8089,443,744
...
[ ! -f ~/recon/$1/oneforall ] && mkdir ~/recon/$1/oneforall  2&>1
...
message () {
	telegram_bot=$(cat ~/hungry-recon/tools/.creds | grep "telegram_bot" | awk {'print $3'})
	telegram_id=$(cat ~/hungry-recon/tools/.creds | grep "telegram_id" | awk {'print $3'})
...
if [ ! -f ~/recon/$1/$1-final.txt ]; then
	...
	if [ ! -f ~/recon/$1/$1-amass.txt ] && [ ! -z $(which amass) ]; then
		amass enum -brute -w ~/tools/SecLists/Discovery/DNS/clean-jhaddix-dns.txt -d $1 -o ~/recon/$1/$1-amass.txt
	...
	if [ ! -f ~/recon/$1/$1-subfinder.txt ] && [ ! -z $(which subfinder) ]; then
		subfinder -d $1 -nW  -rL ~/tools/wordlists/dns/resolvers.txt -o ~/recon/$1/$1-subfinder.txt
	...
	if [ ! -f ~/recon/$1/$1-oneforall.txt ] && [ -e ~/tools/OneForAll ]; then
		cd ~/recon/$1
		python3 ~/tools/OneForAll/oneforall.py --target $1 --brute True --path=./ run
		cat all*.txt >> ~/recon/$1/$1-oneforall.txt

		oneforall=$(scanned ~/recon/$1/$1-oneforall.txt).
		message "oneforall%20Found%20$oneforall%20subdomain(s)%20for%20$1"
		echo "[+] oneforall Found $oneforall subdomains"
	else
		message "[-]%20Skipping%20oneforall%20Scanning%20for%20$1"
		echo "[!] Skipping ..."
	fi
	sleep 5

	
	cat ~/recon/$1/$1-amass.txt ~/recon/$1/$1-oneforall.txt ~/recon/$1/$1-findomain.txt ~/recon/$1/$1-subfinder.txt ~/recon/$1/$1-assetfinder.txt > ~/recon/$1/$1-final-uns.txt | sort -uf
    cat ~/recon/$1/$1-final-uns.txt | sort | uniq > ~/recon/$1/$1-final.txt
    rm ~/recon/$1/$1-final-uns.txt
 	sleep 5
fi
...
if [ ! -f ~/recon/$1/$1-goaltdns.txt ] && [ ! -z $(which goaltdns) ]; then
	goaltdns -l ~/recon/$1/$1-final.txt -w ~/tools/wordlists/dns/commonspeak2-subdomains.txt | ./tools/massdns/bin/massdns -r ~/tools/wordlists/dns/resolvers.txt -o J --flush 2>/dev/null | jq -r '.name' >> ~/recon/$1/$1-goaltdns.tmp
...
if [ ! -f ~/recon/$1/$1-non-wildcards.txt ] && [ ! -z $(which shuffledns) ] && [ ! -z $(which dnsprobe) ]; then
	cat ~/recon/$1/$1-goaltdns.txt ~/recon/$1/$1-final.txt | sed 's/\.$//g' | shuffledns -d $1 -r ~/tools/wordlists/dns/resolvers.txt | dnsprobe -r A | awk {'print $1'} | sort -u >> ~/recon/$1/$1-non-wildcards.txt
...
if [ ! -f ~/recon/$1/$1-masscan.txt ] && [ ! -z $(which masscan) ]; then
	echo $passwordx | sudo -S masscan -p4443,2075,2076,6443,3868,3366,8443,8080,9443,9091,3000,8000,5900,8081,6000,10000,8181,3306,5000,4000,8888,5432,15672,9999,161,4044,7077,4040,9000,8089,443,744,22 -iL ~/recon/$1/$1-ip.txt --max-rate 1000 -oG ~/recon/$1/$1-masscan.txt
...
if [ ! -f ~/recon/$1/$1-tkosubs.txt ] && [ ! -z $(which tko-subs) ]; then
	[ ! -f ~/hungry-recon/tools/providers-data.csv ] && wget "https://raw.githubusercontent.com/anshumanbh/tko-subs/master/providers-data.csv" -O ~/tools/providers-data.csv
	cd ~/
	tko-subs -domains=recon/$1/$1-alive.txt -data=hungry-recon/tools/providers-data.csv -output=recon/$1/$1-tkosubs.txt
...
if [ ! -f ~/recon/$1/$1-subjack.txt ] && [ ! -z $(which subjack) ]; then
	[ ! -f ~/hungry-recon/tools/fingerprints.json ] && wget "https://raw.githubusercontent.com/haccer/subjack/master/fingerprints.json" -O ~/hungry-recon/tools/fingerprints.json
	subjack -w ~/recon/$1/$1-alive.txt -a -timeout 15 -c ~/hungry-recon/tools/fingerprints.json -v -m -o ~/recon/$1/$1-subtemp.txt
	subjack -w ~/recon/$1/$1-alive.txt -a -timeout 15 -c ~/hungry-recon/tools/fingerprints.json -v -m -ssl -o ~/recon/$1/$1-subtmp.txt
...
if [ -e ~/hungry-recon/tools/.tokens ] && [ -f ~/hungry-recon/tools/.tokens ] && [ -f ~/tools/github-search/github-endpoints.py ]; then
	for url in $(cat ~/recon/$1/$1-alive.txt); do 
		echo "Running against: $url"
		python3 ~/tools/github-search/github-endpoints.py -d $url -s -r -t $(cat ~/hungry-recon/tools/.tokens) > ~/recon/$1/github-endpoints/$url.txt
		sleep 7
	done
...
if [ -e ~/hungry-recon/tools/.tokens ] && [ -f ~/hungry-recon/tools/.tokens ] && [ -f ~/tools/github-search/github-secrets.py ]; then
	for url in $(cat ~/recon/$1/$1-alive.txt ); do 
		u=$(echo $url | sed 's/\./\\./g');
		echo "Running against: $url"
		python3 ~/tools/github-search/github-secrets.py -s $u -t $(cat ~/hungry-recon/tools/.tokens) > ~/recon/$1/github-secrets/$url.txt
		sleep 7
	done
...
if [ -f ~/tools/smuggler/smuggler.py ]; then
	for url in $(cat ~/recon/$1/$1-httprobe.txt); do
		filename=$(echo $url | sed 's/http:\/\///g' | sed 's/https:\/\//ssl-/g')
		echo "Running against: $url"
		python3 ~/tools/smuggler/smuggler.py -u "$url/" -v 1 &> ~/recon/$1/http-desync/$filename.txt
	done
...
if [ ! -z $(which aquatone) ]; then
	cat ~/recon/$1/$1-httprobe.txt | aquatone -chrome-path $chromiumPath -ports $big_ports -out ~/recon/$1/aquatone
...
if [ ! -f ~/recon/$1/$1-nmap.txt ] && [ ! -z $(which nmap) ]; then
	[ ! -f ~/hungry-recon/tools/nmap-bootstrap.xsl ] && wget "https://raw.githubusercontent.com/honze-net/nmap-bootstrap-xsl/master/nmap-bootstrap.xsl" -O ~/hungry-recon/tools/nmap-bootstrap.xsl
	echo $passwordx | sudo -S nmap -sSVC -A -O -Pn -p4443,2075,2076,6443,3868,3366,8443,8080,9443,9091,3000,8000,5900,8081,6000,10000,8181,3306,5000,4000,8888,5432,15672,9999,161,4044,7077,4040,9000,8089,443,744 -iL ~/recon/$1/$1-ip.txt --script http-enum,http-title,ssh-brute --stylesheet ~/hungry-recon/tools/nmap-bootstrap.xsl -oA ~/recon/$1/$1-nmap
	nmaps=$(scanned ~/recon/$1/$1-ip.txt)
	xsltproc -o ~/recon/$1/$1-nmap.html ~/hungry-recon/tools/nmap-bootstrap.xsl ~/recon/$1/$1-nmap.xml
...
if [ ! -z $(which webanalyze) ]; then
	[ ! -f ~/hungry-recon/tools/apps.json ] && wget "https://raw.githubusercontent.com/AliasIO/Wappalyzer/master/src/apps.json" -O ~/hungry-recon/tools/apps.json
	for target in $(cat ~/recon/$1/$1-httprobe.txt); do
		filename=$(echo $target | sed 's/http\(.?*\)*:\/\///g')
		webanalyze -host $target -apps ~/hungry-recon/tools/apps.json -output csv > ~/recon/$1/webanalyze/$filename.txt
		sleep 5
	done
